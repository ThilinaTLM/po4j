...
class RealWorldFileTest {
    ...
    void testRoundTripSemanticComparison() throws IOException {
        ...
        // Semantic comparison
        assertEquals(original.size(), reparsed.size(), "Entry count mismatch");
        assertEquals(
        ...
        for (int i = 0; i < original.size(); i++) {
            ...
            assertEquals(orig.getMsgid(), repr.getMsgid(), "Msgid mismatch at entry " + i);
            assertEquals(orig.getMsgstr().orElse(null), repr.getMsgstr().orElse(null), "Msgstr mismatch at entry " + i);
            assertEquals(
                    orig.getMsgctxt().orElse(null), repr.getMsgctxt().orElse(null), "Msgctxt mismatch at entry " + i);
            assertEquals(orig.isPlural(), repr.isPlural(), "Plural mismatch at entry " + i);
            assertEquals(
                    orig.getMsgidPlural().orElse(null),
                    repr.getMsgidPlural().orElse(null),
                    "MsgidPlural mismatch at entry " + i);
            assertEquals(orig.getMsgstrPlural(), repr.getMsgstrPlural(), "MsgstrPlural mismatch at entry " + i);
            assertEquals(orig.getFlags(), repr.getFlags(), "Flags mismatch at entry " + i);
            assertEquals(orig.getReferences(), repr.getReferences(), "References mismatch at entry " + i);
        }
        ...
        for (int i = 0; i < original.getObsoleteEntries().size(); i++) {
            ...
            assertEquals(orig.getMsgid(), repr.getMsgid(), "Obsolete msgid mismatch at entry " + i);
            assertEquals(
                    orig.getMsgstr().orElse(null),
                    repr.getMsgstr().orElse(null),
    ...
    void testRoundTripByteComparison() throws IOException {
        ...
        if (originalContent.equals(output)) {
            ...
            System.out.println("Original: ..."
                    + escape(originalContent.substring(contextStart, Math.min(originalContent.length(), contextEnd)))
                    + "...");
            System.out.println("Output:   ..."
                    + escape(output.substring(contextStart, Math.min(output.length(), contextEnd))) + "...");
